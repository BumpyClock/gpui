...
use gpui::{
    ...
    MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, OverlayInputMode, Pixels,
...
use objc2_app_kit::{
    NSButton as Objc2NSButton, NSView as Objc2NSView, NSWindow as Objc2NSWindow,
...
extern "C" fn reset_cursor_rects(this: &Object, _: Sel) {
    ...
    // whose WINDOW_STATE_IVAR is initialized when the view is created. Any cursor registered
    ...
    unsafe {
        ...
        let cursor: id = match cursor_style {
            ...
            // `None` is implemented with balanced `NSCursor::hide`/`unhide` calls in
            // `MacPlatform::set_cursor_style`. Leaving the view without a cursor rect prevents
            // AppKit from immediately overriding that hidden state.
            CursorStyle::None => return,
        };
