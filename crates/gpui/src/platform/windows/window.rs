...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        if let Some(has_shadow) = params.has_shadow {
            set_window_shadow(hwnd, has_shadow);
        }
...
impl PlatformWindow for WindowsWindow {
    ...
    fn set_visible(&self, visible: bool) {
        ...
        self.0
            ...
            .spawn(async move {
                unsafe {
                    ShowWindowAsync(hwnd, if visible { SW_SHOWNA } else { SW_HIDE })
                        .ok()
                        .context("unable to set window visibility")
                        .log_err();
                }
            })
            .detach();
...
fn set_window_shadow(hwnd: HWND, has_shadow: bool) {
    ...
    unsafe {
        ...
        .context("unable to set window shadow policy")
        .log_err();
    }
}
