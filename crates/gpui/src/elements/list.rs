...
impl ListState {
    ...
    pub fn item_is_above_viewport(&self, ix: usize) -> Option<bool> {
        let state = self.0.borrow();
        let viewport_bounds = state.last_layout_bounds?;
...
mod test {
    ...
    #[test]
    fn uniform_item_height_estimates_full_scroll_extent() {
        let state =
            ListState::new(4, crate::ListAlignment::Top, px(0.)).with_uniform_item_height(px(25.));

        assert_eq!(state.max_offset_for_scrollbar().y, px(100.));

        state.reset_with_uniform_height(3, px(10.));
        assert_eq!(state.max_offset_for_scrollbar().y, px(30.));
    }
